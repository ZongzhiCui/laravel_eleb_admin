...
@section('content')
    <div class="row">
        <div class="col-sm-10">
            ...
            <h5 data-time="{{ $event->prize_date }}" data-is_prize="{{ $event->is_prize }}">开奖时间::{{ $event->prize_date }}------
                <button id="myButton" disabled class="btn btn-sm btn-primary" data-id="{{ $event->id }}">我要开奖了!</button>----------<a href="{{ route('winnersList',compact('event')) }}" class="btn btn-info hidden">获奖名单</a>
            </h5>
...
//            console.log(day,'111',today,'111',tomorrow);
            var is_prize = $('#myButton').closest('h5').data('is_prize');
            if (is_prize == 1){
                $('#myButton').next("a").removeClass('hidden');
            }
